...
it('long - long', () => {
  const x = new Long(-0, 0);
  expect(x).toEqual({ high: 0, low: 0 });

  const x2 = new Long(null, undefined);
  expect(x2).toEqual({ high: 0, low: 0 });
});

it('long - fromInt', () => {
  const x = Long.fromInt(100);
  expect(x).toEqual({ high: 0, low: 100 });

  const y = Long.fromInt(100);
  expect(x === y).toBeTruthy();

  const z = Long.fromInt(129);
  expect(z !== y).toBeTruthy();
});
...
it('long - comparisons', () => {
  for (let i = 0; i < TEST_BITS.length; i += 2) {
    ...
    for (let j = 0; j < TEST_BITS.length; j += 2) {
      ...
      if (i === j) {
        expect(vi.equals(vj)).toBeTruthy();
      }
      if (i !== j) {
        expect(vi.notEquals(vj)).toBeTruthy();
      }
      if (i < j) {
        expect(vi.lessThan(vj)).toBeTruthy();
      }
      if (i <= j) {
        expect(vi.lessThanOrEqual(vj)).toBeTruthy();
      }
      if (i > j) {
        expect(vi.greaterThan(vj)).toBeTruthy();
      }
      if (i >= j) {
        expect(vi.greaterThanOrEqual(vj)).toBeTruthy();
      }
    }
  }
});

it('long - bit operations', () => {
  for (let i = 0; i < TEST_BITS.length; i += 2) {
    const vi = Long.fromBits(TEST_BITS[i + 1], TEST_BITS[i]);
    ...
    for (let j = 0; j < TEST_BITS.length; j += 2) {
      ...
      expect(TEST_BITS[i + 1] & TEST_BITS[j + 1]).toEqual(
    ...
    for (let len = 1; len < 64; len += 1) {
      if (len < 32) {
        ...
        ).toEqual(vi.shiftLeft(len).getHighBits());
        ...
        ).toEqual(vi.shiftRight(len).getLowBits());

        expect(TEST_BITS[i] >>> len).toEqual(
...
const createTestDivMod = (i, count) => {
  const vi = Long.fromBits(TEST_BITS[i + 1], TEST_BITS[i]);
  for (let j = 0; j < TEST_BITS.length; j += 2) {
    const vj = Long.fromBits(TEST_BITS[j + 1], TEST_BITS[j]);
    if (!vj.isZero()) {
...
it('long - div mod', () => {
  ...
  let countDivMod = 0;
  for (let i = 0; i < TEST_BITS.length; i += 2) {
    createTestDivMod(i, countDivMod);
...
it('long - from string', () => {
  for (let i = 0; i < TEST_BITS.length; i += 2) {
    ...
    for (let radix = 2; radix <= 36; radix += 1) {
      ...
      expect(TEST_BITS[i]).toEqual(
        Long.fromString(result, radix).getHighBits(),
      );
      expect(TEST_BITS[i + 1]).toEqual(
        Long.fromString(result, radix).getLowBits(),
      );
    }
  }
});

it('long - doesnt make lazy initializers undefined', () => {
  expect(Long.ZERO).toMatchObject({ low: 0, high: 0 });
  expect(Long.ONE).toMatchObject({ low: 1, high: 0 });
  expect(Long.NEG_ONE).toMatchObject({ low: -1, high: -1 });
  expect(Long.MAX_VALUE).toMatchObject({ low: -1, high: 0x7fffffff });
  expect(Long.MIN_VALUE).toMatchObject({ low: 0, high: 1 << (32 - 1) });
  expect(Long.TWO_PWR_24).toMatchObject({ low: 1 << 24, high: 0 });
});
...
it('long - from number', () => {
  for (let i = 0; i < TEST_BITS.length; i += 2) {
    const num =
      TEST_BITS[i] * 2 ** 32 + TEST_BITS[i + 1] >= 0
        ? TEST_BITS[i + 1]
        : 2 ** 32 + TEST_BITS[i + 1];
  ...
  // Test edge cases
  expect(Long.ZERO).toEqual(Long.fromNumber(NaN));
  expect(Long.ZERO).toEqual(Long.fromNumber(Infinity));
  expect(Long.ZERO).toEqual(Long.fromNumber(-Infinity));
});

it('long - from decimal cached values', () => {
  expect(Long.fromInt(0)).toEqual(Long.fromInt(0.1));
  expect(Long.fromInt(0)).toEqual(Long.fromInt(0.2));
  expect(Long.fromInt(1)).toEqual(Long.fromInt(1.1));
});
...
it('long - base 36 to string', () => {
  expect('zzzzzz').toEqual(Long.fromString('zzzzzz', 36).toString(36));
});

it('long - base default to string', () => {
  expect('0').toEqual(Long.fromString('0xfoobar').toString());
  expect(
    '100000000000000000000000000000000000000000000000000000000000001',
  ).toEqual(Long.fromBits(1, 0x40000000).toString(2));
});
